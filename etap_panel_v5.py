...
# Pardus 25 / Wayland uyumu
os.environ["QT_QPA_PLATFORM"] = "xcb"

# --- Kurumsal Stil ---
STYLE_SHEET = """
QMainWindow { background-color: #f5f6f7; }
QFrame#Header { background-color: #ffffff; border-bottom: 2px solid #3498db; }
...
QPushButton { border-radius: 4px; padding: 10px; font-weight: bold; border: 1px solid #dcdde1; background-color: white; }
...
class EtapKilitPaneli(QMainWindow):
    # Thread-Safe UI güncellemeleri için sinyaller
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("TSOMTAL ETAP Merkezi Yönetim Sistemi v6")
        self.setMinimumSize(1200, 800)
        self.setStyleSheet(STYLE_SHEET)

        # Veri Yükleme (Varsayılan maskeli ağ eklendi)
    ...
    def init_ui(self):
        ...
        if not pix.isNull():
            logo.setPixmap(pix.scaled(75, 75, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
        ...
        self.clock_lbl.setStyleSheet("font-size: 18px; font-weight: bold; color: #2c3e50;")
        ...
        self.ip_in.setPlaceholderText("Örn: 10.46.197.0/24")
        ...
        def run():
            for i in range(1, 255):
                ...
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.settimeout(0.04) # Daha hızlı tarama
    ...
    def execute_ssh(self, ip, action):
        ...
        try:
            ...
            ssh.connect(ip, username=user, password=pw, timeout=4)
            ...
            if action == "lock":
                ...
                # Kilit Açma: Cinnamon screensaver ve session unlock birleşimi
                cmd = (
                    "export DISPLAY=:0; "
